...
void createStudentsFromFile(char* fileName, struct student** head, struct student** headErrors) { // this function read objects from file, organize them into a list and print it
    FILE* in;
    ...
    while (!feof(in)) {                                                        // read file until end 
        ...
        free(line);
        validateNewStruct(&newStruct, head, headErrors);
    }
    printTitles();
    printList(*head);
...
struct student* createStruct(char* line) {
    ...
    newStruct->next = NULL;
    newStruct->c_language = INITIAL_GRADE;                      
    newStruct->computer_network = INITIAL_GRADE;
    newStruct->cs_fundementals = INITIAL_GRADE;
    int grade;
    char* fix = NULL;
    ...
    sscanf(line, " %s%s%s%s%d", newStruct->fname, newStruct->lname, newStruct->ID, newStruct->course, &grade);
    ...
    int numOfTests = ((newStruct->c_language != INITIAL_GRADE) + (newStruct->computer_network != INITIAL_GRADE) + (newStruct->cs_fundementals != INITIAL_GRADE));
...
void validateNewStruct(struct student** newStudent, struct student** head, struct student** headErrors) {
...
void writeTofile(struct student* newStruct, char* fileName) {
    ...
    int grade = newStruct->c_language != INITIAL_GRADE ? newStruct->c_language : newStruct->cs_fundementals != INITIAL_GRADE ? newStruct->cs_fundementals : newStruct->computer_network;
